...
describe("CircleSplit Contracts", function () {
  ...
  describe("LedgerFactory", function () {
    ...
    it("Should track multiple ledgers per user", async function () {
      await ledgerFactory.createLedger(
        "Ledger 1",
        APPROVE_AMOUNT,
        MAX_DAILY,
        MAX_MONTHLY
      ...
      const ledger1 = await ledgerFactory.getUserLedgerByIndex(
        owner.address,
        0
      );
      const ledger2 = await ledgerFactory.getUserLedgerByIndex(
        owner.address,
        1
      );
    ...
    describe("Member Tracking", function () {
      ...
      beforeEach(async function () {
        ...
        ledgerAddress = await ledgerFactory.getUserLedgerByIndex(
          owner.address,
          0
        );
      });
      ...
      it("Should track multiple ledgers for a user", async function () {
        ...
        const secondLedgerAddress = await ledgerFactory.getUserLedgerByIndex(
          owner.address,
          1
        );
        ...
        await mockUSDC
          .connect(user1)
          .approve(secondLedgerAddress, APPROVE_AMOUNT);

        await ledger1.connect(user1).join();
        await ledger2.connect(user1).join();

        // Check tracking
        const memberLedgers = await (ledgerFactory as any).getUserMemberLedgers(
          user1.address
        );
        ...
        expect(
          await (ledgerFactory as any).getUserMemberLedgersCount(user1.address)
        ).to.equal(2);
      });
      ...
      it("Should provide access to member ledgers by index", async function () {
        ...
        const firstMemberLedger = await (
          ledgerFactory as any
        ).getUserMemberLedgerByIndex(user1.address, 0);
      ...
      it("Should only allow valid ledgers to call member tracking functions", async function () {
        await expect(
          (ledgerFactory as any).addMemberToLedger(
            user1.address,
            ethers.ZeroAddress
          )
        ).to.be.reverted;
        ...
        await expect(
          (ledgerFactory as any).removeMemberFromLedger(
            user1.address,
            ethers.ZeroAddress
          )
        ).to.be.reverted;
  ...
  describe("Ledger", function () {
    beforeEach(async function () {
      ...
      const ledgerAddress = await ledgerFactory.getUserLedgerByIndex(
        owner.address,
        0
      );
    ...
    describe("Initialization", function () {
      it("Should initialize with correct parameters", async function () {
        ...
        const [approveAmount, maxDaily, maxMonthly] =
          await ledger.getSettings();
    ...
    describe("Member Management", function () {
      it("Should allow users to join after approving tokens", async function () {
        ...
        await mockUSDC
          .connect(user1)
          .approve(await ledger.getAddress(), APPROVE_AMOUNT);
      ...
      it("Should revert if user hasn't approved enough tokens", async function () {
        await expect(ledger.connect(user1).join()).to.be.revertedWith(
          "Approve USDC first"
        );
      });
      ...
      it("Should revert if non-member tries to leave", async function () {
        await expect(ledger.connect(user1).leave()).to.be.revertedWith(
          "Not a member"
        );
      });
    });
    ...
    describe("Payment Splitting", function () {
      ...
      it("Should split payment successfully", async function () {
        ...
        await expect(
          ledger
            .connect(user1)
            .splitPayment(externalId, vendor.address, participants, amounts)
      ...
      it("Should handle failed transfers gracefully", async function () {
        ...
        await mockUSDC
          .connect(user3)
          .transfer(owner.address, await mockUSDC.balanceOf(user3.address));
        ...
        expect(paymentInfo.failedAmounts[0]).to.equal(
          ethers.parseUnits("30", 6)
        );

        // Check successful participants
        const successfulParticipants = await ledger.getSuccessfulParticipants(
          0
        );
      ...
      it("Should revert for non-members", async function () {
        ...
        await expect(
          ledger
            .connect(owner)
            .splitPayment(12348, vendor.address, participants, amounts)
      ...
      it("Should revert with invalid parameters", async function () {
        ...
        await expect(
          ledger
            .connect(user1)
            .splitPayment(12349, vendor.address, participants, amounts)
    ...
    describe("Usage Tracking", function () {
      ...
      it("Should track usage correctly", async function () {
        ...
        await ledger
          .connect(user1)
          .splitPayment(12352, vendor.address, participants, amounts);

        const [dailyStart, dailyAccumulated] = await ledger.getDailyUsage(
          user1.address
        );
        const [monthlyStart, monthlyAccumulated] = await ledger.getMonthlyUsage(
          user1.address
        );
    ...
    describe("Payment History", function () {
      ...
      it("Should return failed details correctly", async function () {
        ...
        await mockUSDC
          .connect(user2)
          .transfer(owner.address, await mockUSDC.balanceOf(user2.address));
        ...
        const [failedParticipants, failedAmounts] =
          await ledger.getFailedDetails(0);
